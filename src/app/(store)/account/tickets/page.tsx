    ...
    <div className="mx-auto max-w-4xl px-4 pt-8 sm:px-6 sm:pt-12">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-bold">تذاكر الدعم</h1>
          <p className="mt-2 text-sm text-muted">محادثاتك مع فريق الدعم. نرسل لك بريداً عند كل رد.</p>
        </div>
      ...
      {tickets.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            title={total > 0 ? "لا توجد تذاكر في هذه الصفحة" : "لا توجد تذاكر بعد"}
            description="واجهتك مشكلة في طلب أو لديك سؤال؟ افتح تذكرة وسنرد عليك."
          >
            <Link href={total > 0 ? "/account/tickets" : "/support"} className="btn-primary">
              {total > 0 ? "العودة لأول صفحة" : "افتح تذكرة"}
            </Link>
          </EmptyState>
        </div>
      ) : (
              ...
              >
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block text-xs text-muted">
                    آخر رسالة <LocalTime iso={t.lastMessageAt.toISOString()} /> ·{" "}
      ...
      <Pager pathname="/account/tickets" param="page" params={{}} page={page} pageSize={PAGE_SIZE} total={total} label="صفحات التذاكر" />
    </div>
  );
}
