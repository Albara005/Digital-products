...
const perks = [
  { icon: IconReceipt, text: "كل طلباتك ومنتجاتك في مكان واحد" },
  { icon: IconWallet, text: "محفظة رصيد للدفع بضغطة واحدة" },
  { icon: IconBolt, text: "إتمام أسرع للطلبات القادمة" },
];

export default async function LoginPage({ searchParams }: Props) {
  const next = safeNextPath(firstParam((await searchParams).next));
  if (await getSignedInCustomer()) redirect(next);
    ...
    <div className="mx-auto grid max-w-5xl gap-8 px-4 pt-8 sm:px-6 sm:pt-14 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)] lg:items-center lg:gap-14">
      <section className="order-2 lg:order-1" aria-labelledby="login-perks">
        ...
        <h2 id="login-perks" className="mt-3 text-2xl leading-tight font-bold sm:text-3xl">
          حسابك في Nitro Store
        </h2>
        <p className="mt-3 max-w-md text-sm leading-7 text-muted sm:text-base">
          لا حاجة لكلمة مرور: أدخل بريدك وسنرسل لك رمز دخول صالحاً لمدة 10 دقائق. إن لم يكن لديك حساب فسننشئه
          تلقائياً بنفس البريد الذي تستلم عليه طلباتك.
        </p>
