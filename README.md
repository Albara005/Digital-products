...
- **العرض والدفع بعملة العميل**: تُحدَّد العملة تلقائياً (SAR، AED، KWD، QAR، BHD، OMR، EGP أو USD افتراضياً) ويرى العميل كل الأسعار بها ويدفع بها فعلياً، دون أي سعر تقريبي.
- **محفظة العميل**: رصيد يُحفظ بالدولار ويُعرض بعملة العميل، يُشحن عبر بوابة الدفع ويُستخدم كلياً أو جزئياً في الشراء، بسجل حركات لا يُعدَّل وقفل على مستوى قاعدة البيانات يمنع صرف الرصيد مرتين.
...
- أسعار المنتجات وقيم الكوبونات وإعدادات الدعوة والمحفظة تُخزَّن كلها **بالدولار (سنتات)**. تُحدَّد عملة الزائر تلقائياً من بلده (رأس CDN أو لغة المتصفح، ثم المنطقة الزمنية في المتصفح) في الكوكي `nitro_currency`، ويمكنه تغييرها من رأس المتجر. الدولار هو الافتراضي.
- عند اختيار عملة تُعرض كل الأسعار (البطاقات، صفحة المنتج، السلة، الطلب، الحساب، المحفظة، الشحن، البريد) **ويتم الدفع** بتلك العملة. التحويل في دالة واحدة `toMinor()`/`convertUsdCents()` في `src/lib/display-currency.ts`: `round(السنتات/100 × سعر الصرف × 10^الخانات)` بوحدات العملة الصغرى، فالمعروض هو المدفوع تماماً. العملات ذات 3 خانات (KWD/BHD/OMR) تُقرَّب لأقرب 0.010 لأن Stripe يشترط أن تنتهي مبالغها بصفر.
- `POST /api/checkout` يقرأ العملة من الحقل `currency` (يجب أن تكون مفعّلة) أو من الكوكي، ويحوّل كل سطر والخصم (الكوبون الثابت والحد الأدنى والحد الأقصى تُحوَّل من الدولار، والنسبة كما هي) والمحفظة والإجمالي. الطلب يُحفظ بعملته: كل حقول `*Cents` فيه بوحدات عملته الصغرى، مع `fxRate` (وحدات لكل دولار) و`totalUsdCents` (القيمة بالدولار) و`walletDebitUsdCents` (ما خُصم من المحفظة بالدولار). Tap وStripe يُنشئان الدفع بعملة الطلب وخاناتها، والحد الأدنى للدفع ≈ 0.50 دولار محوّلاً (AED 2.00 على الأقل).
- التقارير ولوحة التحكم و"المُنفَق" ومكافآت الدعوة تجمع `totalUsdCents`. لوحة التحكم لها عملة عرض خاصة (قائمة في الشريط الجانبي، كوكي `nitro_admin_currency`): تُعرض المجاميع بها بسعر الصرف الحالي، وتُدخَل الأسعار والمبالغ بها وتُخزَّن بالدولار `round(القيمة / السعر × 100)`، بينما يظهر كل طلب بعملته التي دُفع بها.
- أسعار الصرف تُعدَّل من **الإعدادات → العملات** وتسري على الطلبات الجديدة فقط؛ كل طلب يحتفظ بسعر الصرف الذي دُفع به (ويُستخدم عند الاسترجاع إلى المحفظة).

## المحفظة

- رصيد العميل يُحفظ بالدولار ويُعرض بعملته، ويظهر مع سجل حركاته في صفحة حسابه. كل حركة (شحن، شراء، استرجاع، تعديل من الإدارة) تُسجَّل مع الرصيد بعدها، والرصيد لا يصبح سالباً أبداً حتى مع طلبات متزامنة.
- الشحن من صفحة الحساب عبر `POST /api/wallet/topup` بعملة العميل (بمبالغ صحيحة تعادل من 5 إلى 500 دولار، ويتطلب تسجيل الدخول): يُدفع بعملته ويُضاف للمحفظة ما يعادله بالدولار بسعر الصرف لحظة الشحن، مرة واحدة فقط عند تأكيد الدفع من Tap أو Stripe.
- عند الشراء يختار العميل المسجّل "استخدام رصيد المحفظة" فيُخصم منه حتى قيمة الطلب ويُدفع الباقي عبر البوابة. في طلب بعملة أخرى يُخصم من المحفظة ما يعادل الجزء المستخدم بالدولار مقرّباً للأسفل (لا يُخصم أكثر مما عُرض)، والاسترجاع إلى المحفظة يعيد القيمة بالدولار بسعر صرف الطلب نفسه.
...
## البريد الإلكتروني (Gmail أو Resend)

بدون إعداد بريد لا تصل رموز الدخول للعملاء (تُكتب في سجلات الخادم فقط). اختر طريقة:

**Gmail (الأسهل، بدون دومين):** فعّل التحقق بخطوتين في حساب جوجل، ثم من **Security → App passwords** أنشئ كلمة مرور تطبيق (16 حرفاً)، وأضف:
```
...
```
لا تضبط `EMAIL_FROM` مع Gmail. الحد تقريباً 500 رسالة يومياً.

**Resend (للإنتاج مع دومين خاص):**
...
- Wallet (`src/lib/wallet.ts`): append-only ledger, customer row locked for every movement, never negative. `POST /api/wallet/topup` `{ amountCents, currency?, provider? }` (session required): `amountCents` in minor units of the customer's currency, whole units, worth $5–$500 (`topupLimits()`); charged in that currency, credits the USD equivalent (`WalletTopup.creditUsdCents`, fixed at creation) once on confirmed payment.
- Currencies: catalog prices, coupon amounts, referral settings and the wallet are USD cents. The shopper's currency (auto-detected, cookie `nitro_currency`, USD by default) is used for display **and** charging: `toMinor()` / `convertUsdCents()` in `src/lib/display-currency.ts` = round(usdCents/100 × rate × 10^decimals), rounded to 0.010 for 3-decimal currencies (Stripe requires a trailing 0). Orders store `currency`, all `*Cents` fields in that currency's minor units, `fxRate`, `totalUsdCents` and `walletDebitUsdCents`; reports, dashboard, "spent" and referral rewards sum `totalUsdCents`. The admin panel has its own display currency (cookie `nitro_admin_currency`): aggregates are converted at the current rate, money inputs are typed in it and stored as USD cents. Unit tests: `node --import tsx --test scripts/verify-fx.mts`.
